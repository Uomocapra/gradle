...
<verification-metadata xmlns="https://schema.gradle.org/dependency-verification"
      ...
      xsi:schemaLocation="https://schema.gradle.org/dependency-verification https://schema.gradle.org/dependency-verification/dependency-verification-1.4.xsd">
   <configuration>
...
====
include::sample[dir="snippets/securing-builds/dependencyVerification-disableVerification/kotlin",files="build.gradle.kts[tags=disable-verification]"]
include::sample[dir="snippets/securing-builds/dependencyVerification-disableVerification/groovy",files="build.gradle[tags=disable-verification]"]
====
...
A `pgp` entry also accepts optional `origin` and `reason` attributes to document where the key was obtained or verified and why it is trusted:
...
As with checksums, these attributes are informational only: Gradle preserves them across read/write cycles but doesn't use them during verification.
...
* `origin` - an optional, free-form note documenting where the key was obtained or verified (for example, the URL it was downloaded from)
* `reason` - an optional note explaining why the key is trusted
...
The `origin` and `reason` attributes are purely informational: Gradle preserves them across read/write cycles but never uses them during verification.
